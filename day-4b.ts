...
const boardsWon = new Array<boolean>();

console.log(lastWinningScore());

function lastWinningScore() {
  ...
  for (let i = 0; (i < numbers.length); i++) {
    ...
    for (let b = 0; (b < boards.length); b++) {
      if (boardWins(boards[b], soFar)) {
        boardsWon[b] = true;
        if (!boardsWon.some(value => value === false)) {
          return boardScore(boards[b], soFar);
        }
      }
    }
  }
